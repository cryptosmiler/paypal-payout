<header class="flex items-center justify-between px-6 py-4 bg-white border-b-4 border-indigo-600">
    <div class="flex items-center">
        ...
        @if(auth()->guard("admin")->check() && auth()->guard("admin")->user()->email_verified_at && Route::currentRouteName() !== "dashboard")
        ...
        <a href="{{route('dashboard')}}">
            <img src="{{ asset('assets/svg/logo.svg') }}" alt="SVG Image" class="w-[50px] h-[30px]">
        </a>
        @endif
    </div>
    ...
    <div class="flex items-center gap-2">
        ...
        <div x-data="{ dropdownOpenLang: false }"  class="relative">
            <button @click="dropdownOpenLang = ! dropdownOpenLang" class="relative block w-28 h-8 overflow-hidden rounded-full shadow focus:outline-none" >
                {{ Config::get('languages')[App::getLocale()] }}
            </button>

            <div x-cloak x-show="dropdownOpenLang" @click="dropdownOpenLang = false" class="fixed inset-0 z-10 w-full h-full"></div>

            <div x-cloak x-show="dropdownOpenLang" class="absolute right-0 z-10 w-28 mt-2 overflow-hidden bg-white rounded-md shadow-xl">
                @foreach (Config::get('languages') as $lang => $language)
                    @if ($lang != App::getLocale())
                        <a href="{{ route('locale.change', $lang) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">{{ $language }}</a>
                    @endif
                @endforeach
            </div>
        </div>

        @if(auth()->guard("admin")->check() && auth()->guard("admin")->user()->email_verified_at)
        ...
        <div x-data="{ dropdownOpen: false }"  class="relative">
            <button @click="dropdownOpen = ! dropdownOpen" class="relative block w-8 h-8 overflow-hidden rounded-full shadow focus:outline-none">
                ...
                <img class="object-cover w-full h-full" src="{{ url(auth()->guard('admin')->user()->avatar) }}" alt="Your avatar">
                @else 
            ...
            <div x-cloak x-show="dropdownOpen" class="absolute right-0 z-10 w-28 mt-2 overflow-hidden bg-white rounded-md shadow-xl">
                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white"> {{ app('language')['profile'] }} </a>
                <a href="{{ route('auth.logout') }}?localTime=localTime" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white"> {{ app('language')['logout'] }} </a>
            </div>
        </div>
        @else 
            @if(Route::currentRouteName() == "login.show")
            <a class="flex mx-4 text-gray-600 focus:outline-none" href="{{route('register.show')}}">
                {{ app('language')['register'] }}
            </a>
            @elseif(Route::currentRouteName() != "login.show")
            <a class="flex mx-4 text-gray-600 focus:outline-none" href="{{route('login.show')}}">
                {{ app('language')['login'] }}
            </a>
            @endif
        @endif
