...
@section('body')
    <h3 class="text-gray-700 text-3xl font-medium"> {{ app('language')['edit_landing_item'] }} </h3>
    ...
    <div class="mt-8">
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <form action="{{ route('landingItem.update', $landingItem->id) }}" method="POST" class="mt-4" enctype="multipart/form-data"  >
                @csrf
                {{method_field('PATCH')}}
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    ...
                    <div >
                        ...
                        <x-locale-select sLocale="{{ old('locale', $landingItem->locale) }}" />
                    </div>

                    <div class=" col-span-1 sm:col-span-2">
                        <x-video-input
                            name="file"
                            videoSrc="{{ $landingItem->video }}"
                        />
                    </div>
                    ...
                    <div>
                        ...
                        <input name="title" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text" value="{{ old('title', $landingItem->title) }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" >{{ old('description', $landingItem->description) }}</textarea>
                        @error('description')
                            <span class=" text-red-600" role="alert">
                                {{ $message }}
                            </span>
                    ...
                    <div>
                        ...
                        <input name="order" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="number" value="{{ old('order', $landingItem->order) }}">
                        @error('order')
                            <span class=" text-red-600" role="alert">
                                {{ $message }}
                            </span>
