...
@section('body')
    <div class="flex flex-row justify-between">
        <h3 class="text-gray-700 text-3xl font-medium"> {{ app('language')['edit_subject'] }} </h3>
    </div>
    ...
    <div class="mt-8">
        <div class="p-6 bg-white rounded-md shadow-md">
            ...
            <form action="{{ route('subject.update', $subject->id) }}" method="POST" class="mt-4" enctype="multipart/form-data"  >
                @csrf
                {{method_field('PATCH')}}
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    <div >
                        ...
                        <x-locale-select sLocale="{{ old('locale', $subject->locale) }}" />
                    </div>
                    ...
                    <div>
                        ...
                        <input name="title" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" type="text" value="{{ old('title', $subject->title) }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea name="description" class="form-input w-full mt-2 rounded-md focus:border-indigo-600" >{{ old('description', $subject->description) }}</textarea>
                        @error('description')
                            <span class=" text-red-600" role="alert">
                                {{ $message }}
                            </span>
                ...
                <div class="flex justify-end mt-4 gap-2">
                    ...
                    <a href="{{ route('subject.index') }}" class="px-4 py-2 bg-gray-800 text-gray-200 rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-700" > {{ app('language')['cancel'] }} </a>
                </div>
            </form>
        </div>
    </div>
